...
- Consumer-defined store interfaces: `auth.Store`, `workers.Store`, `domain.SettingsReader`. The monolithic `store.Store` interface is deleted. `internal/app/app.go` is the only package that imports the concrete `sqlite.Store`.
