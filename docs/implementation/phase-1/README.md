...
- PostgreSQL is the only Phase 1 database implementation. sqlc generates queries from `internal/store/postgres/queries/*.sql`. The `store/` package owns the concrete Postgres store.
