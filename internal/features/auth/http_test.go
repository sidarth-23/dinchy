...
func (r testSettingsReader) Bootstrap(context.Context) (bootstrap.BootstrapState, error) {
	return r.state, nil
}
...
func newHTTPTestAPI(t *testing.T) (*API, *MockStore) {
	...
	api := &API{
		auth:         svc,
		settings:     testSettingsReader{state: bootstrap.BootstrapState{InstanceName: "dinchy"}},
		requireHTTPS: false,
	}
	return api, store
}
...
func TestAPISession_ReturnsCurrentViewer(t *testing.T) {
	...
	ctx := support.WithSession(testCtx, &session.SessionWithUser{
		SessionID:     "s1",
		UserID:        "u1",
