...
func TestResolve_LocalizesAndPreservesMeta(t *testing.T) {
	...
	resp := renderer.Resolve(language.English, apperrors.Conflict(i18n.Msg(i18n.CodeAuthSetupCompleted, i18n.P("resource", "users"), i18n.P("count", 2))))
...
func TestResolve_ServerErrorRendersGenericButPreservesCode(t *testing.T) {
	...
	renderer := apperrors.NewRenderer(i18n.Default, false)
	appErr := apperrors.Internal(i18n.Msg(i18n.CodeAuthLoginFindUser), apperrors.WithCause(assert.AnError))
	resp := renderer.Resolve(language.English, appErr)

	assert.Equal(t, http.StatusInternalServerError, resp.GetStatus())
	assert.Equal(t, string(i18n.CodeServerInternalError), resp.Payload.Code)
	...
	assert.Equal(t, i18n.CodeAuthLoginFindUser, appErr.Code())
}
...
func TestResolve_ExposeInternalAttachesDebug(t *testing.T) {
	...
	assert.Equal(t, string(i18n.CodeAuthLoginFindUser), resp.Payload.Debug.Code)
...
func TestResolve_ExposeInternalOnClientErrorAttachesDebug(t *testing.T) {
	...
	renderer := apperrors.NewRenderer(i18n.Default, true)
	appErr := apperrors.BadRequest(i18n.Msg(i18n.CodeAuthOrganisationNotFound))
	resp := renderer.Resolve(language.English, appErr)

	assert.Equal(t, http.StatusBadRequest, resp.GetStatus())
	assert.Equal(t, string(i18n.CodeAuthOrganisationNotFound), resp.Payload.Code)
	require.NotNil(t, resp.Payload.Debug)
	assert.Equal(t, string(i18n.CodeAuthOrganisationNotFound), resp.Payload.Debug.Code)
}
